...
const handler = async (req, res) => {
  if (req.method === 'POST') {
    ...
    let htmlContent = `<!doctypehtml><html lang=en><body style="margin:0 auto 0 auto;width:500px;font-family:'trebuchet ms';background-color:#fff!important"><div><div style=text-align:center><img alt="Wandering Jade Logo"src="https://www.wanderingjademobilebar.com/wandering-jade-4.png"style=width:200px;height:200px></div><div style=text-align:center;padding:10px><h1 style="margin:0 auto 0 auto;width:fit-content;border-radius:15px;background-color:#e6bdbe;padding:.75rem;color:#fff">New Quote!</h1></div><div style="padding:10px 30px"><p>Name: <span style=font-weight:700;font-size:larger>${data.name}</span><p>Email: <span style=font-weight:700;font-size:larger>${data.email}</span><p>Phone: <span style=font-weight:700;font-size:larger>${data.phone}</span><p>Date: <span style="font-weight: bold; font-size: larger;"><a href="https://calendar.google.com/event?action=TEMPLATE&dates=${newDate}/${newDate}&text=${data.name}&details=${data.event}">${data.date}</a></span></p><p>Address: <span style=font-weight:700;font-size:larger>${data.address}</span><p>Type of Event: <span style=font-weight:700;font-size:larger>${data.event}</span><p>Coordinator Info (if Applicable): <span style=font-weight:700;font-size:larger>${data.weddingDetails}</span><p>Guests: <span style=font-weight:700;font-size:larger>${data.count}</span><p>Package: <span style=font-weight:700;font-size:larger>${data.package}</span><p>Social: <span style=font-weight:700;font-size:larger>${data.social}</span><p>Additional Notes: <span style=font-weight:700;font-size:larger>${data.notes}</span><p>Addons:</p><ul>`
